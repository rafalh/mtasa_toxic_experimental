...
function Animation.presets.guiMoveEx(endX, endY, time, loop, startX, startY, strEasingType)
	-- guiMoveEx(endX, endY, [ time = 1000, loop = false, startX = current X, startY = current Y, strEasingType = "Linear" ])
	if type(endX) ~= 'userdata' then
		return { from = 0, to = 1,
		         time = time or 1000, repeats = loop and 0 or 1, fn = Animation.presets.guiMoveEx,
